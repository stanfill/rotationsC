...
\end{equation}
An algorithm proposed by \citet{hartley11} is employed by the function \code{median} with option \code{type="geometric"}.  The \dfn{geometric mean} is the $L_2$ analog of $\GeoMedian$ given by 
\begin{equation}\label{est:ltwo}
\GeoMean=\argmin_{\bm{S}\in
SO(3)}\sum_{i=1}^n\Rdist^2(\bm{R}_i,\bm{S}).
\end{equation}
The function \code{mean} with option \code{type="geometric"} implements an algorithm first proposed by \citet{manton04} in estimating $\GeoMean$.
