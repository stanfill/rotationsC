...
opts_chunk$set(fig.path='figures/', fig.align='center', fig.show='hold',comment=NA)
...
Asymptotic results for the projected mean $\ProjMean$ can be used to perform inference for the central orientation $\bm S$. In particular, results in \cite{prentice1986,chang2001} and Zhang et al. (2009) give three distinct approaches to this problem.  We make these approaches available within the \code{region} function through the \code{method} argument.
