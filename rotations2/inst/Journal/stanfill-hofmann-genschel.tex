...
The function \code{SO3} will creates a rotation matrix according to \eqref{eqn:angleaxis} for a variety of inputs.  One can supply an angle and unit-axis separately, or as a single three dimensional axis with length equal to the angle.  One can also supply a rotation in the quaternion representation and the matrix equivalent will be return as an \code{SO3} object.  All objects are returned as vectors with the number of rows equal to the sample size.  See the following for the construction of a rotation about the $y$-axis through an angle uniformly chosen on the interval $[0,\pi]$.
