...
A multitude of methods for location models are available in R, but for the case of $SO(3)$ data the toolbox is more limited.  The \CRANpkg{orientlib} package includes the definition of an orientation class along with a few methods to summarize and visualize rotation data \citep{murdoch2003}.  Though the representations of rotations are examined in detail in the \CRANpkg{orientlib} package, only one method for summarizing orientation data is available and the visualizations offered are not intuitive.
