...
\section{$SO(3)$ Data generation} 

{\color{red} MAKE A TABLE LIKE IN THE TECHNO PAPER AND SIMPLIFY THIS SECTION SIGNIFICANTLY}

%\begin{center} 
\begin{table}[h!]
\caption{Circular densities with respect to the Lebesgue measure and circular variance $\nu$.}  \label{tab:ang.dens}\centering
\small{
\begin{tabular}{ lclcl}\toprule
\textbf{Name} & & \textbf{Density} $C(r |\kappa)$ & & \textbf{Circular variance $\nu$}\\ \midrule 
\rule[2mm]{0mm}{6mm} Uniform & & $\frac{1-\cos(r)}{2\pi}$ & & $\frac{3}
{2}$ \\
...
{\color{red} AGAIN, MAKE A TABLE SUMMARIZING THE ESTIMATORS AND SIMPLIFY THE REST OF THIS SECTION}
...
\midrule
\rule[2mm]{0mm}{6mm} Projected Arithmetic Mean & & $\ProjMean$ & & \code{"projected"} &&$\sum_{i=1}^n d^2_E$  \\
