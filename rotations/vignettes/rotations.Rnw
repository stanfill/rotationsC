...
\usepackage{booktabs}
%\VignetteIndexEntry{rotations Overview}
...
\usepackage{bm,subcaption,amsfonts,natbib}
...
library(rotations)
options(digits=3)
opts_chunk$set(fig.path='figure/', fig.width=5, fig.height=5, fig.show='hold')
@
...
%\begin{article}
\begin{center}
{\Large\pkg{rotations}: An R Package for $SO(3)$ Data}
...
<<ex7,fig.cap="The $x$-axis of a random sample from the Cayley-UARS distribution with $\\kappa=1$, $n=50$.  All for point estimates are displayed in (a) and all three region methods along with the projected mean are in (b).",fig.lp="figure:eye1",out.width=".4\\textwidth">>=
